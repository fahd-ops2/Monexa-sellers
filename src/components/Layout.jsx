...
function Layout({ title, children, activeTab, onTabChange, theme, onThemeToggle }) {
  return (
    <div className="min-h-screen flex flex-col bg-slate-50 dark:bg-slate-900 transition-colors">
      ...
      <header className="sticky top-0 z-10 flex-shrink-0 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 px-4 py-4 md:px-6 md:py-5 shadow-sm dark:shadow-none transition-colors">
        <div className="flex items-center justify-between gap-3 max-w-4xl mx-auto">
          <h1 className="text-xl md:text-2xl font-semibold text-slate-900 dark:text-slate-100 truncate">
            {title}
          </h1>
          {onThemeToggle && (
